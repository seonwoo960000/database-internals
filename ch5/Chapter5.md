...
- Page cache 기능 
  - 메모리에 페이지를 저장 -> 캐싱된 페이지의 접근 속도 향상  
  - 페이지의 수정사항을 디스크에 반영하기 전 버퍼링을 통해 성능을 최적화
  - 페이지 eviction 알고리즘을 통해 중요도가 낮은 페이지를 메모리에서 제거 
- O_DIRECT 
  - I/O system call을 사용할 때 kernel page cache를 거치지 않도록하는 flag 
    - DB에 최적화된 buffer management를 사용하기 위해 사용됨 
  - 단점 
    - 동기적으로 처리됨 
...
- Caching 
  ...
  - Pinning 
    - 메모리에 로드된 page를 evict되지 않도록하는 방법 
    - B-Tree의 경우 상위 레벨의 노드일수록 자주 접근됨 -> pinning했을 때 성능 향상에 도움 
  - Dirty 
...
- WAL(Write Ahead Log)
  ...
  - Checkpointing 
    - WAL에 존재하는 로그 중 디스크의 page에 반영이 완료된 로그를 삭제 
- Log Semantics 
  - LSN 
    - Log Sequence Number 
    - 단조 증가하는, 로그별로 유일한 숫자 
    - 로그를 저장하는 과정에서도 log buffer을 통해 최적화를 진행 
      ...
      - transaction은 관련된 작업이 모두 디스크상의 로그에 반영되지 않은 경우 commit 됐다고 간주될 수 없음 
  - CLR
    - Compensation Log Records 
      - 데이터베이스 복구 중 recovery 작업을 수행하는 과정에서도 별도의 로그를 기록 -> CLR 
  - Checkpoint
    - 디스크의 log에서 실제 page에 반영된 로그를 표시하는 작업 
  - Fuzzy checkpoints 
...
- Concurrency control 방법 개요 
  ...
  - MVCC(MultiVersion Concurrency Control)
    ...
    - lockless(timestamp ordering) 또는 lock based(2PL) 방법이 존재 
  - PCC(Pessimistic Concurrency Control)
    - 트랜잭션이 실행되는 시점에 계속 트랜잭션간 충돌이 있는지 확인 
    - lockless 
...
- Serializability 
  ...
  - complete schedule 
    - schedule에 포함된 transaction에 포함된 모든 작업이 수행된 경우
  - serial
    - schedule에 포함된 transaction들의 작업이 교차하지 않고 실행된 경우를 의미 
...
- PCC 
  ...
  - lock-free 
    ...
    - 쓰기 작업을 수행하려는 tx's timestamp < 쓰기 대상 데이터의 max_write_timestamp
      - Thomas write rule으로 인해 abort 하지 않음 
      - 단, 해당 쓰기 작업은 무시할 수 있음 
  - lock-based 
    - 2PL 
...
- Deadlock을 해소하는 방법 
  - timeout 
  - abort long runnint transactions 
- Deadlock 감지 
  - waits-for graph 
- Deadlock 발생 시 어떤 트랜잭션을 abort? 
  - Wait-die 
    - higher timestamp를 지닌 tx만이 현재 트랜잭션을 blocking할 수 있음 
  - Wound-wait 
    - lower timestamp를 지닌 tx만이 현재 트랜잭션을 blocking할 수 있음 
## Locks vs Latches  
- Locks 
  - 논리적 잠금 
- Latches 
